...
func TestClientV7_Count(t *testing.T) {
	rt := roundTripper(func(r *http.Request) *http.Response {
		ok := func(code int, body string) *http.Response {
			resp := &http.Response{StatusCode: code, Status: http.StatusText(code), Body: io.NopCloser(bytes.NewBufferString(body)), Header: make(http.Header)}
			resp.Header.Set("X-Elastic-Product", "Elasticsearch")
...
func TestClientV7_DeleteByQuery(t *testing.T) {
	rt := roundTripper(func(r *http.Request) *http.Response {
		ok := func(code int, body string) *http.Response {
			resp := &http.Response{StatusCode: code, Status: http.StatusText(code), Body: io.NopCloser(bytes.NewBufferString(body)), Header: make(http.Header)}
			resp.Header.Set("X-Elastic-Product", "Elasticsearch")
...
func TestClientV7_Refresh(t *testing.T) {
	rt := roundTripper(func(r *http.Request) *http.Response {
		ok := func(code int, body string) *http.Response {
			resp := &http.Response{StatusCode: code, Status: http.StatusText(code), Body: io.NopCloser(bytes.NewBufferString(body)), Header: make(http.Header)}
			resp.Header.Set("X-Elastic-Product", "Elasticsearch")
...
func TestClientV7_NewClientV7(t *testing.T) {
	es, err := esv7.NewClient(esv7.Config{Addresses: []string{"http://mock"}})
	require.NoError(t, err)
	c := NewClientV7(es)
	require.NotNil(t, c)
}

func TestClientV7_ErrorCases(t *testing.T) {
