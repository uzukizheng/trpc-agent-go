...
import (
	...
	"trpc.group/trpc-go/trpc-agent-go/core/agent/parallelagent"
...
func (c *parallelChat) setup(ctx context.Context) error {
	...
	marketAgent := llmagent.New(
		...
		llmagent.WithSystemPrompt("Market analyst: focus on trends, competition, market size, customer demand, and industry dynamics. Provide specific data."),
	...
	technicalAgent := llmagent.New(
		...
		llmagent.WithSystemPrompt("Technical expert: analyze feasibility, requirements, implementation details, technology stack, and technical challenges."),
		llmagent.WithGenerationConfig(genConfig),
		llmagent.WithChannelBufferSize(50),
		llmagent.WithTools([]tool.Tool{}),
	)
	...
	riskAgent := llmagent.New(
		...
		llmagent.WithSystemPrompt("Risk analyst: identify risks, challenges, compliance issues, security concerns, and provide mitigation strategies."),
		llmagent.WithGenerationConfig(genConfig),
		llmagent.WithChannelBufferSize(50),
		llmagent.WithTools([]tool.Tool{}),
	)
	...
	opportunityAgent := llmagent.New(
		...
		llmagent.WithSystemPrompt("Opportunity analyst: identify benefits, strategic advantages, cost savings, revenue potential, and ROI opportunities."),
		llmagent.WithGenerationConfig(genConfig),
		llmagent.WithChannelBufferSize(50),
		llmagent.WithTools([]tool.Tool{}),
	)
